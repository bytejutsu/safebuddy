...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _saveEmail() async {
    ...
    try {
      ...
      setState(() => _editEmail = false);
      Get.snackbar(
          'Confirm email change', 'Check your inbox to confirm the new login email.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _green,
          colorText: Colors.white,
  ...
  // ── Extracted Obx widgets ────────────────────────────────────────────────

  /// Refresh button with loading indicator — owns its own Obx scope.
  Widget _buildHistoryRefreshButton() {
    return Obx(() {
      ...
      return GestureDetector(
        onTap: () => ctrl.fetchHistory(),
        child: Icon(Icons.refresh, size: 20, color: _blue.withOpacity(0.6)),
      );
    });
  }

  /// History list body — owns its own Obx scope.
  Widget _buildHistoryList() {
    return Obx(() {
      final ctrl = ContactsController.to;
      if (ctrl.isHistoryLoading.value && ctrl.history.isEmpty) {
      ...
      return ListView.separated(
        ...
        itemBuilder: (_, i) => _buildHistoryTile(ctrl.history[i]),
      );
    });
  }
                    ...
                    children: [
                      const Text('Sharing History',
                          style: TextStyle(
                              ...
                              fontWeight: FontWeight.bold,
                              color: _blue)),
                      // ✅ Extracted into its own method — clean Obx scope
                      _buildHistoryRefreshButton(),
                    ],
                  ),
                  const SizedBox(height: 10),
                  Expanded(
