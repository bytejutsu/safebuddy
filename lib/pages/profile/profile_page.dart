...
class _ProfilePageState extends State<ProfilePage> {
  ...
  bool get _isProfileComplete => _name.trim().isNotEmpty && _phoneVerified;
  ...
  Future<void> _loadProfile() async {
    try {
      ...
      final Map<String, dynamic>? profile =
          refreshedRows.isNotEmpty ? refreshedRows.first : null;
  ...
  Future<void> _updateProfile(Map<String, dynamic> values) async {
    final user = _currentUser;
    if (user == null) { Get.offAllNamed('/signin'); return; }
  ...
  Future<void> _saveName(String value) async {
    ...
    try {
      ...
      Get.snackbar('Saved', 'Your name has been updated.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _green, colorText: Colors.white);
    } catch (e) {
      Get.snackbar('Error', 'Could not save your name.',
          snackPosition: SnackPosition.BOTTOM,
  ...
  Future<void> _savePhone() async {
    ...
    try {
      setState(() => _isSaving = true);
      await _updateProfile({'phone': trimmed, 'phone_verified': verified});
      if (!mounted) return;
      setState(() {
        _phone = trimmed;
      ...
      Get.snackbar('Saved', 'Your phone number has been updated.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _green, colorText: Colors.white);
    } catch (e) {
      Get.snackbar('Error', 'Could not save your phone number.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _red, colorText: Colors.white);
    } finally {
      if (mounted) setState(() => _isSaving = false);
  ...
  Future<void> _saveEmail() async {
    ...
    if (newEmail.isEmpty) {
      Get.snackbar('Invalid email', 'Email cannot be empty.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _red, colorText: Colors.white);
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _editEmail = false);
      Get.snackbar('Confirm email change',
      ...
      if (!mounted) return;
      setState(() { _emailCtrl.text = currentEmail; _editEmail = false; });
      ...
      if (!mounted) return;
      setState(() { _emailCtrl.text = currentEmail; _editEmail = false; });
      Get.snackbar('Error', 'Could not update your email.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _red, colorText: Colors.white);
    } finally {
      if (mounted) setState(() => _isSaving = false);
      ...
      builder: (ctx) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            ListTile(
              leading: const CircleAvatar(backgroundColor: _blue,
            ...
            ListTile(
              leading: const CircleAvatar(backgroundColor: _indigo,
            ...
            if (_avatarUrl.isNotEmpty)
              ListTile(
                leading: const CircleAvatar(backgroundColor: _red,
  ...
  Future<void> _pickFromSource(ImageSource source) async {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        await _supabase.storage.from('avatars').uploadBinary(storagePath, bytes,
            fileOptions: const FileOptions(cacheControl: '3600', upsert: true));
      } else {
        await _supabase.storage.from('avatars').upload(storagePath, File(picked.path),
            fileOptions: const FileOptions(cacheControl: '3600', upsert: true));
      }
      ...
      if (!mounted) return;
      setState(() => _avatarUrl = versionedUrl);
      Get.snackbar('Saved', 'Your profile photo has been updated.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _green, colorText: Colors.white);
    } catch (e) {
      Get.snackbar('Error', 'Could not upload the image.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _red, colorText: Colors.white);
    } finally {
      if (mounted) setState(() => _isSaving = false);
  ...
  Future<void> _removeAvatar() async {
    ...
    try {
      setState(() => _isSaving = true);
      await _supabase.storage.from('avatars').remove(['${user.id}/avatar.jpg']);
      await _updateProfile({'avatar_url': null});
      if (!mounted) return;
      setState(() => _avatarUrl = '');
      Get.snackbar('Removed', 'Your profile photo has been removed.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _green, colorText: Colors.white);
    } catch (e) {
      Get.snackbar('Error', 'Could not remove the profile photo.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: _red, colorText: Colors.white);
    } finally {
      if (mounted) setState(() => _isSaving = false);
  ...
  Widget _buildAvatar() {
    Widget avatarContent = _avatarUrl.isNotEmpty
        ? ClipOval(
            child: Image.network(_avatarUrl, width: 90, height: 90,
                fit: BoxFit.cover,
  ...
  Widget _buildRow(IconData icon, String label, TextEditingController ctrl,
      String hint, bool editing, TextInputType kb,
      VoidCallback onEdit, VoidCallback onDone) {
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: editing
                ? TextField(
                    controller: ctrl, autofocus: true, keyboardType: kb,
          ...
          GestureDetector(
            ...
            child: Container(width: 32, height: 32,
              ...
              child: Icon(editing ? Icons.check : Icons.edit,
                  size: 16, color: editing ? _green : _blue))),
        ],
      ),
    );
  }
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: Icon(
              isActive ? Icons.location_on_rounded : Icons.location_off_outlined,
              size: 20,
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        _buildRow(Icons.phone_outlined, 'Phone', _phoneCtrl,
                            'xx xxx xxx', _editPhone, TextInputType.phone,
                            () => setState(() => _editPhone = true),
                            () => _savePhone()),
                        Divider(height: 1, indent: 16, endIndent: 16,
                            color: Colors.grey[100]),
                        _buildRow(Icons.email_outlined, 'Email', _emailCtrl,
                            'your@email.com', _editEmail,
                  ...
                  Row(
                    ...
                    children: [
                      const Text('Sharing History',
                          style: TextStyle(fontSize: 16,
                              fontWeight: FontWeight.bold, color: _blue)),
                      Obx(() => ContactsController.to.isHistoryLoading.value
                          ...
                          : GestureDetector(
                              ...
                              child: Icon(Icons.refresh,
                                  size: 20, color: _blue.withOpacity(0.6)))),
                    ],
                  ),
                  const SizedBox(height: 10),
                  Expanded(
                      ...
                      child: Obx(() {
                        ...
                        if (ctrl.history.isEmpty) {
                          return Column(
                            ...
                            children: [
                              ...
                              Text('Share your location with a contact\nto see it here',
                        ...
                        return ListView.separated(
                          ...
                          separatorBuilder: (_, __) => Divider(
                              ...
                              color: Colors.grey[100]),
                          itemBuilder: (_, i) =>
                              _buildHistoryTile(ctrl.history[i]),
                        );
                      }),
                    ),
                  ),
